...
CRYPTO_PAIRS = [
    ...
    'TON/USDT', 'DOT/USDT', 'LTC/USDT', 'BCH/USDT', 'NEAR/USDT',
    'APT/USDT', 'ARB/USDT', 'OP/USDT', 'ATOM/USDT', 'FIL/USDT',
    'INJ/USDT', 'SUI/USDT', 'SEI/USDT', 'TIA/USDT', 'WLD/USDT',
    'ORDI/USDT', 'ENA/USDT', 'WIF/USDT', 'PEPE/USDT', 'ONDO/USDT',
    'HYPE/USDT', 'JUP/USDT', 'RUNE/USDT', 'AAVE/USDT', 'UNI/USDT',
    'FET/USDT', 'GALA/USDT', 'CRV/USDT', 'LDO/USDT', 'PAXG/USDT',
]
...
# TRUE = wajib garis K menyilang D (ketat). FALSE = cukup masuk zona jenuh (longgar).
REQUIRE_STOCH_CROSS = env_bool("REQUIRE_STOCH_CROSS", True)
STOCH_PERIOD = env_int("STOCH_PERIOD", 7)
...
# --- PATCH_TPSL_V1: jaring pengaman stop loss ---
MIN_SL_PCT = env_float("MIN_SL_PCT", 0.004)      # jarak SL minimum dari entry (0.4%)
SL_GUARD_SEC = env_int("SL_GUARD_SEC", 30)       # ronda pemeriksa SL di bursa
ADOPT_ORPHAN = env_bool("ADOPT_ORPHAN", True)    # ambil alih posisi yatim di bursa
# --- PATCH_NOTIFSYNC_V1: notif hanya setelah bursa konfirmasi ---
CLOSE_CONFIRM_TRIES = env_int("CLOSE_CONFIRM_TRIES", 5)      # percobaan cek posisi nol
CLOSE_CONFIRM_DELAY = env_float("CLOSE_CONFIRM_DELAY", 1.2)  # jeda antar pengecekan
USE_MARK_TRIGGER = env_bool("USE_MARK_TRIGGER", True)        # trigger pakai MARK, bukan LAST
...
KIMI_MODEL = env_str("KIMI_MODEL", "moonshotai/kimi-k2.6")
KIMI_API_KEY = env_str("OPENROUTER_API_KEY_KIMI") or env_str("OPENROUTER_API_KEY")
KIMI_REASONING = env_bool("KIMI_REASONING", True)
KIMI_MAX_TOKENS = env_int("KIMI_MAX_TOKENS", 700)   # reasoning butuh ruang lebih

# --- AGEN 2: DEEPSEEK V4 FLASH (EvoMap, endpoint & key sendiri) ---
EVOMAP_MODEL = env_str("EVOMAP_MODEL", "evomap-deepseek-v4-flash")
...
# v4-flash adalah model reasoning: butuh token lega, kalau sempit content kosong.
EVOMAP_MAX_TOKENS = env_int("EVOMAP_MAX_TOKENS", 1500)
KIMI_INTERVAL_SEC = env_int("KIMI_INTERVAL_SEC", 900)
...
DASHBOARD_ROWS = env_int("DASHBOARD_ROWS", 0)      # 0 = ikut jumlah pair
DASHBOARD_COLS = env_int("DASHBOARD_COLS", 0)      # 0 = otomatis ikut lebar terminal
HEARTBEAT_SEC = env_int("HEARTBEAT_SEC", 60)       # ringkasan berkala saat mode log (PM2)
# "smart" = tampilkan pair yang menarik saja + ringkasan; "full" = semua pair
RADAR_MODE = env_str("RADAR_MODE", "smart").strip().lower()
UI_REFRESH = env_int("UI_REFRESH_PER_SEC", 4)
# Aksesibilitas: NO_COLOR menonaktifkan warna, UI_ASCII untuk terminal tanpa unicode
UI_NO_COLOR = env_bool("UI_NO_COLOR", False) or bool(os.environ.get("NO_COLOR"))
UI_ASCII = env_bool("UI_ASCII", False)
...
CONSOLE = Console(no_color=UI_NO_COLOR, soft_wrap=False) if RICH_OK else None
...
CLIENT_KIMI = _buat_klien(KIMI_API_KEY, OPENROUTER_BASE)
CLIENT_LLAMA = _buat_klien(LLAMA_API_KEY, OPENROUTER_BASE)
CLIENT_EVOMAP = _buat_klien(evomap_key(), EVOMAP_BASE)
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if res.status_code == 200:
                return True
            log.error("Telegram HTTP %s: %s", res.status_code, res.text[:200])
            if res.status_code == 400:  # markdown rusak -> kirim plain
                payload.pop("parse_mode", None)
        except Exception as exc:
            log.error("Telegram error (attempt %s): %s", attempt, exc)
...
PROMPT_KIMI = (
    "Anda adalah Direktur Strategi Makro Kuantitatif Hedge Fund. Analisis struktur tren "
    "futures BTC dan logam global. Tentukan rezim pasar saat ini. Balas HANYA JSON dengan key "
    "'market_regime' (BULLISH_TREND/BEARISH_TREND/SIDEWAYS_CHOP), 'recommended_tactic' "
    "(MEAN_REVERSION_ONLY/TREND_FOLLOWING_AGGRESSIVE), 'global_bias_rationale' (1 kalimat)."
)
...
async def agent1_kimi_macro():
    ...
    try:
        ...
        AI_STATUS["kimi"] = "OK"
        MARKET_REGIME_GLOBAL = {
            "market_regime": data.get("market_regime", "SIDEWAYS_CHOP"),
        ...
        log.info("KIMI regime -> %s", MARKET_REGIME_GLOBAL["market_regime"])
        push_event("REGIME", f"Kimi: {MARKET_REGIME_GLOBAL['market_regime']} · "
                             f"{MARKET_REGIME_GLOBAL['recommended_tactic']}")
    except Exception as exc:
        AI_STATUS["kimi"] = "ERROR"
        MARKET_REGIME_GLOBAL["last_updated"] = "FALLBACK"
        log.error("Agent1 Kimi gagal: %s", exc)


def _chat_evomap(prompt: str) -> dict:
    """Jalur khusus EvoMap deepseek-v4-flash (model reasoning).

    Tanpa system message dan tanpa response_format, persis gaya pemanggilan
    yang terbukti jalan. Token dilonggarkan supaya penalaran tidak memakan
    habis jatah sebelum jawaban ditulis.
    """
    if CLIENT_EVOMAP is None:
        raise RuntimeError("klien EvoMap tidak aktif (API key kosong?)")
    res = CLIENT_EVOMAP.chat.completions.create(
        model=EVOMAP_MODEL,
        messages=[{"role": "user", "content": prompt}],
        max_tokens=EVOMAP_MAX_TOKENS,
        temperature=0.2,
    )
    ch = res.choices[0]
    isi = (ch.message.content or "").strip()
    if not isi:
        # Jaring pengaman: kalau jawaban nyangkut di kolom penalaran, panen dari sana.
        isi = (getattr(ch.message, "reasoning_content", "") or "").strip()
        if isi:
            log.warning("EvoMap content kosong (finish=%s) -> panen reasoning_content",
                        getattr(ch, "finish_reason", "?"))
    return _ekstrak_json(isi)


def _wajib_putusan(data, sumber: str) -> dict:
    """Pastikan balasan model benar-benar punya putusan. Kalau tidak,
    itu kegagalan driver (HTML/teks kosong), bukan REJECT yang sah."""
    if not isinstance(data, dict) or not str(data.get("decision", "")).strip():
        cuplikan = str(data)[:180].replace("\n", " ")
        raise ValueError("%s balas tanpa key decision -> %s" % (sumber, cuplikan))
    return data


def _baca_putusan_agent2(data: dict, tag: str):
    ok = str(data.get("decision", "")).upper() == "CONFIRMED"
    log.info("DEEPSEEK putusan=%s skor=%s | %s", data.get("decision"),
             data.get("sentiment_score", "-"), data.get("audit_log", "-"))
    return (ok, f"{tag}{data.get('audit_log', '-')}")
...
async def agent2_deepseek(symbol: str, side: str, m: dict, funding: float):
    ...
    prompt = (
        f"Anda auditor eksekusi scalping timeframe 3 menit untuk {symbol} arah {side}.\n"
        f"Data: harga {m['live']}, open candle {m['open']}, ATR {m['atr']:.6f}, "
        f"StochRSI K {m['k']:.1f} D {m['d']:.1f}, funding rate {funding:.6f}.\n"
        "Target profit hanya 0.22% dengan stop 0.9x ATR, jadi sinyal TIDAK perlu sempurna "
        "dan tidak perlu tren besar. Tugas Anda menyaring jebakan nyata saja.\n"
        "Balas CONFIRMED bila minimal DUA benar: (1) K dan D searah sinyal, "
        "(2) harga sudah bergerak >= 0.25x ATR dari open candle, "
        "(3) funding tidak melawan arah lebih dari 0.05 persen.\n"
        "Balas REJECT HANYA bila ada bukti jebakan kuat: funding ekstrem melawan arah "
        "di atas 0.1 persen, ATR nol, atau harga stagnan total.\n"
        "PENTING: funding kecil yang melawan arah BUKAN alasan REJECT. "
        "Kondisi overbought/oversold adalah ALASAN UTAMA sinyal ini ada, bukan kelemahan.\n"
        "Balas HANYA JSON dengan key 'decision' (CONFIRMED/REJECT), "
        "'sentiment_score' (1-100), 'audit_log' (1 kalimat singkat)."
    )
    if CLIENT_EVOMAP is not None:
        try:
            data = await asyncio.to_thread(_chat_evomap, prompt)
            data = _wajib_putusan(data, "EvoMap")
    ...
    if client_or is None or not EVOMAP_FALLBACK_MODEL:
        AI_STATUS["deepseek"] = "OFF"
        return (AI_FAIL_OPEN, "EvoMap mati & tidak ada fallback")
    try:
        data = await asyncio.to_thread(_or_json, EVOMAP_FALLBACK_MODEL, prompt, 220)
        data = _wajib_putusan(data, "fallback " + str(EVOMAP_FALLBACK_MODEL))
        AI_STATUS["deepseek"] = "FALLBACK"
...
async def agent3_llama(symbol: str, side: str, m: dict, book_summary: str):
    ...
    prompt = (
        f"Anda Hakim Agung Eksekutor bot V15. Audit order book {symbol} untuk {side}. "
        f"Ringkasan kedalaman: {book_summary}. Harga {m['live']}, ATR {m['atr']:.6f}. "
        "Balas HANYA JSON dengan key 'veto_decision' (VETO_EXECUTE/VETO_ABORT) dan "
        "'veto_reason' (1 kalimat)."
    )
    try:
        data = await asyncio.to_thread(
            _chat_json, CLIENT_LLAMA, LLAMA_MODEL, prompt, 200, _extra_llama())
        AI_STATUS["llama"] = "OK"
        ok = str(data.get("veto_decision", "")).upper() == "VETO_EXECUTE"
...
async def ai_alliance(symbol: str, side: str, m: dict):
    ...
    if not AI_LAYER_ENABLED:
        return True, "AI layer disabled (raw sensor mode)"
    funding = await ambil_funding(symbol)
    ok2, log2 = await agent2_deepseek(symbol, side, m, funding)
    if not ok2:
        return False, f"DeepSeek REJECT: {log2}"
    book = await ringkas_order_book(symbol)
    ok3, log3 = await agent3_llama(symbol, side, m, book)
    if not ok3:
        return False, f"Llama VETO_ABORT: {log3}"
    return True, log3
...
def hitung_target(side: str, entry: float, atr: float):
    # PATCH_TPSL_V1: jarak SL tidak boleh lebih tipis dari MIN_SL_PCT.
    # ATR nyaris nol (TON, PAXG) dulu membuat stopPrice menempel di entry dan
    # ditolak bursa ("order would immediately trigger").
    jarak = max(SL_ATR_MULT * atr, entry * MIN_SL_PCT)
    if side == "LONG":
        tp = entry * (1 + TP_PCT)
        sl = entry - jarak
    else:
...
async def eksekusi_order(symbol: str, side: str, m: dict):
    ...
    tp_id = sl_id = None
    # PATCH_TPSL_V1: TP boleh gagal, SL TIDAK BOLEH. Tanpa SL posisi ditutup.
    try:
    ...
    lebar = abs(entry - sl)
    for percobaan in (1, 2, 3):
        sl_coba = (entry - lebar) if side == "LONG" else (entry + lebar)
        try:
            sl_order = await EXCHANGE.create_order(
                symbol, "STOP_MARKET", close_side, filled, None,
                {"stopPrice": float(EXCHANGE.price_to_precision(symbol, sl_coba)),
                 "reduceOnly": True, "workingType": "MARK_PRICE"})
            sl_id = sl_order.get("id")
            sl = sl_coba
            break
        except Exception as exc:
            log.error("Gagal pasang SL %s (percobaan %s, jarak %.8f): %s",
                      symbol, percobaan, lebar, exc)
            lebar *= 2.0
            await asyncio.sleep(0.6)

    if not sl_id and not await ada_sl_di_bursa(symbol):
        log.error("SL TIDAK BISA DIPASANG %s -> posisi ditutup paksa", symbol)
        push_event("FAIL", f"{symbol} SL gagal dipasang -> posisi ditutup paksa")
        try:
            await EXCHANGE.create_order(symbol, "market", close_side, filled, None,
                                        {"reduceOnly": True})
        except Exception as exc2:
            log.error("Force close %s setelah SL gagal: %s", symbol, exc2)
        try:
            await EXCHANGE.cancel_all_orders(symbol)
        except Exception:
            pass
        await tg_send("\u26d4 *ENTRY DIBATALKAN* `%s` %s\nStop loss tidak bisa dipasang "
                      "di bursa, posisi langsung ditutup. Bot menolak memegang posisi "
                      "tanpa jaring pengaman." % (symbol, side))
        raise RuntimeError("SL gagal dipasang -> posisi ditutup, entry dibatalkan")
...
# --- TELEMETRI GERBANG (patch_gatestat) ---
GATE_STAT_EVERY = env_int("GATE_STAT_EVERY", 20)
GATE_STAT = {}
GATE_SCAN = [0]


def _stat(k: str):
    GATE_STAT[k] = GATE_STAT.get(k, 0) + 1


def _lapor_gate():
    GATE_SCAN[0] += 1
    if GATE_STAT_EVERY <= 0 or GATE_SCAN[0] % GATE_STAT_EVERY != 0:
        return
    total = sum(GATE_STAT.values()) or 1
    urut = sorted(GATE_STAT.items(), key=lambda x: -x[1])
    rinci = " · ".join(
        "%s=%d(%.0f%%)" % (k, v, v * 100.0 / total) for k, v in urut)
    log.info("GATE STAT scan#%d | %s", GATE_SCAN[0], rinci)
...
async def proses_pair(symbol: str, board: dict):
    ...
    side = None
    if m["live"] >= barrier_up and m["k"] > STOCH_OB and (m["k"] < m["d"] or not REQUIRE_STOCH_CROSS) and regime != "BULLISH_TREND":
        side = "SHORT"
    elif m["live"] <= barrier_dn and m["k"] < STOCH_OS and (m["k"] > m["d"] or not REQUIRE_STOCH_CROSS) and regime != "BEARISH_TREND":
        side = "LONG"

    if side is None:
        if symbol in REGISTRY.open:
            tag = "IN POSITION"
            _stat("in_position")
        else:
            _lb = abs(m["live"] - m["open"]) >= (ATR_MULTIPLIER * m["atr"])
            _ls = (m["k"] > STOCH_OB) or (m["k"] < STOCH_OS)
            if _lb and _ls:
                _ar = "SHORT" if m["live"] > m["open"] else "LONG"
                _cocok = ((_ar == "SHORT" and m["k"] > STOCH_OB)
                          or (_ar == "LONG" and m["k"] < STOCH_OS))
                tag = "CROSS BLOCK" if _cocok else "ARAH CLASH"
                _stat("cross_block" if _cocok else "arah_clash")
            elif _lb:
                tag = "STOCH"
                _stat("stoch_saja")
            elif _ls:
    ...
    # GATE JAM PASAR TRADFI: XAU/XAG/XPT dilarang entry saat pasar logam tutup
    ...
    if gate is not None:
        _stat("gate:" + str(gate).split("_")[0])
        log.info("BLOKIR %s %s | gate=%s", symbol, side, gate)
        board[symbol] = f"🔒 {gate}"
        return

    if not await REGISTRY.reserve(symbol):
        _stat("slot_race")
        log.info("BLOKIR %s %s | slot race", symbol, side)
        board[symbol] = "🔒 SLOT RACE"
        return
    REGISTRY.mark_signal(symbol)
    _stat("MASUK_AI")
    log.info("MASUK AI %s %s | K=%.1f D=%.1f", symbol, side, m["k"], m["d"])
    ...
    try:
        ...
        if not approved:
            ...
            push_event("VETO", f"{symbol} {side} ditolak AI: {str(alasan)[:150]}")
...
async def tutup_posisi(pos: dict, exit_price: float, reason: str):
    symbol = pos["symbol"]
    if not pos["paper"]:
        # PATCH_TPSL_V1: batalkan HANYA order milik posisi ini.
        # cancel_all_orders dulu menyapu TP/SL posisi lain pada simbol sama
        # (saat dua proses bot jalan) sehingga posisi lain jadi telanjang.
        for kunci in ("tp_id", "sl_id"):
            oid = pos.get(kunci)
            if not oid:
                continue
            try:
                ...
                log.warning("cancel %s %s: %s", kunci, symbol, exc)
...
async def harga_mark(symbol: str):
    """PATCH_NOTIFSYNC_V1: harga MARK dari bursa -- acuan asli trigger TP/SL.

    Order STOP/TP dipasang dengan workingType MARK_PRICE, jadi membandingkan
    target dengan harga LAST membuat bot menutup pembukuan lebih dulu daripada
    bursa. Kembalikan None kalau tidak bisa dibaca.
    """
    try:
        t = await EXCHANGE.fetch_ticker(symbol)
    except Exception as exc:
        log.warning("baca mark %s: %s", symbol, exc)
        return None
    info = t.get("info") or {}
    for kunci in ("markPrice", "mark_price"):
...
async def cek_posisi_bursa(symbol: str):
    """PATCH_NOTIFSYNC_V1: True=masih ada, False=sudah nol, None=TIDAK PASTI.

    Bedanya dengan posisi_masih_terbuka: galat jaringan tidak dianggap "sudah
    tertutup". Ragu harus berarti ragu, bukan alasan mengirim notif LOSE.
    """
    try:
        positions = await EXCHANGE.fetch_positions([symbol])
    except Exception as exc:
        log.warning("cek posisi %s: %s", symbol, exc)
        return None
    for p in positions:
...
async def tunggu_posisi_tertutup(symbol: str) -> bool:
    """PATCH_NOTIFSYNC_V1: True hanya kalau bursa benar-benar bilang posisi nol."""
    for _ in range(max(1, CLOSE_CONFIRM_TRIES)):
        status = await cek_posisi_bursa(symbol)
        if status is False:
            return True
...
async def paksa_market_close(pos: dict):
    """PATCH_NOTIFSYNC_V1: kembalikan harga fill NYATA (atau None).

    Dulu balikannya dibuang dan pembukuan memakai harga wick lokal, sehingga
    angka PnL di notif tidak sama dengan yang terjadi di bursa.
    """
    if pos["paper"]:
        return None
    side = "sell" if pos["side"] == "LONG" else "buy"
    try:
        order = await EXCHANGE.create_order(pos["symbol"], "market", side, pos["amount"],
                                            None, {"reduceOnly": True})
    except Exception as exc:
        log.error("Force close %s gagal: %s", pos["symbol"], exc)
        return None
    try:
        harga = order.get("average") or order.get            ("price")
        return float(harga) if harga else None
    except Exception:
        return None
...
async def ada_sl_di_bursa(symbol: str) -> bool:
    """PATCH_TPSL_V1: tanya bursa langsung, apakah masih ada order STOP."""
    try:
        orders = await EXCHANGE.fetch_open_orders(symbol)
    except Exception as exc:
        log.warning("fetch_open_orders %s: %s", symbol, exc)
        return True      # ragu -> anggap ada, jangan tutup posisi karena galat jaringan
    for o in orders:
...
async def jaga_sl(pos: dict):
    """PATCH_TPSL_V1: SL hilang -> pasang ulang; tetap gagal -> tutup posisi."""
    if pos.get("paper"):
        return
    symbol, side = pos["symbol"], pos["side"]
    if await ada_sl_di_bursa(symbol):
        return
    close_side = "sell" if side == "LONG" else "buy"
    log.warning("SL HILANG %s -> dipasang ulang", symbol)
    try:
        order = await EXCHANGE.create_order(
            symbol, "STOP_MARKET", close_side, pos["amount"], None,
            {"stopPrice": float(EXCHANGE.price_to_precision(symbol, pos["sl"])),
             "reduceOnly": True, "workingType": "MARK_PRICE"})
        pos["sl_id"] = order.get("id")
        push_event("INFO", f"{symbol} SL dipasang ulang di {fmt(pos['sl'])}")
        await tg_send("\U0001f6e1\ufe0f *SL DIPASANG ULANG* `%s` %s di `%s`"
                      % (symbol, side, fmt(pos["sl"])))
        return
    except Exception as exc:
        log.error("Pasang ulang SL %s gagal: %s", symbol, exc)
    push_event("FAIL", f"{symbol} tanpa SL -> ditutup paksa")
    await paksa_market_close(pos)
    harga = float(pos.get("last_price") or pos["entry"])
    await tutup_posisi(pos, harga, "TUTUP PAKSA (SL HILANG)")


async def adopsi_posisi(p: dict):
    """PATCH_TPSL_V1: posisi ada di bursa tapi tak dikenal bot -> ambil alih."""
    symbol = p.get("symbol")
    ...
    tp, sl = hitung_target(side, entry, 0.0)
    pos = {
        "symbol": symbol, "side": side, "entry": entry, "amount": qty,
        "tp": tp, "sl": sl, "tp_id": None, "sl_id": None,
        "breakeven_done": False, "paper": False,
        "opened_at": datetime.now(timezone.utc).isoformat(),
        "order_id": "ADOPSI",
    ...
    await REGISTRY.commit(symbol, pos)
    log.warning("ADOPSI posisi yatim %s %s qty=%s entry=%s", symbol, side, qty, entry)
    push_event("INFO", f"Adopsi posisi yatim {symbol} {side} qty {qty}")
    await tg_send("\U0001f504 *POSISI YATIM DIADOPSI* `%s` %s\nqty `%s` \u00b7 entry `%s`\n"
                  "SL `%s` \u00b7 TP `%s`"
                  % (symbol, side, qty, fmt(entry), fmt(sl), fmt(tp)))
    await jaga_sl(pos)
    ...
    while True:
        try:
            for symbol, pos in list(REGISTRY.open.items()):
                ...
                # PATCH_TPSL_V1: ronda SL tiap SL_GUARD_SEC detik
                if not pos.get("paper"):
                    now_ts = datetime.now(timezone.utc).timestamp()
                    if now_ts - float(pos.get("sl_check_ts") or 0) >= SL_GUARD_SEC:
                        pos["sl_check_ts"] = now_ts
                        try:
                            await jaga_sl(pos)
                        except Exception as exc:
                            log.error("jaga_sl %s: %s", symbol, exc)
                        if symbol not in REGISTRY.open:
                            continue
                side = pos["side"]
                ...
                if TRADFI_FORCE_CLOSE and is_tradfi(symbol):
                    ...
                    if not masih_buka:
                        # PATCH_NOTIFSYNC_V1: konfirmasi dulu sebelum membukukan.
                        harga_fill = await paksa_market_close(pos)
                        if not pos["paper"] and not await tunggu_posisi_tertutup(symbol):
                            log.error("TRADFI close %s belum tertutup -> notif ditahan", symbol)
                            push_event("FAIL", f"{symbol} tradfi close gagal -> notif ditahan")
                            continue
                        await tutup_posisi(pos, float(harga_fill or price),
                                           f"TRADFI MARKET CLOSE ({alasan_pasar})")
                        continue
                ...
                # Deteksi TP / SL -> catat WIN/LOSS  [PATCH_NOTIFSYNC_V1]
                ...
                if hit_tp or hit_sl:
                    reason = "TAKE PROFIT" if hit_tp else (
                        "BREAKEVEN STOP" if pos["breakeven_done"] else "STOP LOSS")
                    if pos["paper"]:
                        await tutup_posisi(pos, price, reason)
                        continue

                    # (a) Bursa memicu dengan MARK_PRICE. Harga LAST sering
                    #     menyentuh lebih dulu -> dulu bot kirim notif LOSE
                    #     padahal di app belum terjadi apa pun.
                    if USE_MARK_TRIGGER:
                        mark = await harga_mark(symbol)
                        if mark is not None:
                            hit_tp = mark >= pos["tp"] if side == "LONG" else mark <= pos["tp"]
                            hit_sl = mark <= pos["sl"] if side == "LONG" else mark >= pos["sl"]
                            if not (hit_tp or hit_sl):
                                continue      # cuma wick harga last -> diamkan
                            reason = "TAKE PROFIT" if hit_tp else (
                                "BREAKEVEN STOP" if pos["breakeven_done"] else "STOP LOSS")

                    # (b) Tutup dulu di bursa, baru pembukuan. Simpan harga fill.
                    harga_fill = None
                    if await cek_posisi_bursa(symbol) is not False:
                        harga_fill = await paksa_market_close(pos)

                    # (c) WAJIB: notif WIN/LOSE hanya setelah bursa bilang nol.
                    if not await tunggu_posisi_tertutup(symbol):
                        log.error("BELUM TERTUTUP di bursa %s -> notif WIN/LOSE ditahan", symbol)
                        push_event("FAIL", f"{symbol} belum tertutup di bursa -> notif ditahan")
                        pos["sl_check_ts"] = 0.0      # paksa ronda SL memeriksa segera
                        continue

                    exit_price = harga_fill
                    if not exit_price:
                        exit_price = await harga_mark(symbol) or price
                    await tutup_posisi(pos, float(exit_price), reason)
                    continue
                ...
                if age > MAX_TRADE_AGE_SEC:
                    # PATCH_NOTIFSYNC_V1: sama seperti TP/SL, konfirmasi dulu.
                    harga_fill = await paksa_market_close(pos)
                    if not pos["paper"] and not await tunggu_posisi_tertutup(symbol):
                        log.error("TIME STOP %s belum tertutup -> notif ditahan", symbol)
                        push_event("FAIL", f"{symbol} time stop belum tertutup -> notif ditahan")
                        continue
                    await tutup_posisi(pos, float(harga_fill or price), "TIME STOP")
        except Exception as exc:
            log.error("watcher error: %s", exc)
        await asyncio.sleep(2.0)


async def posisi_masih_terbuka(symbol: str) -> bool:
    try:
        positions = await EXCHANGE.fetch_positions([symbol])
        for p in positions:
            if abs(float(p.get("contracts") or 0)) > 0:
...
async def sync_exchange_positions():
    ...
    while True:
        ...
        try:
            ...
            if ADOPT_ORPHAN:      # PATCH_TPSL_V1: posisi yatim diambil alih + diberi SL
                for p in positions:
                    if abs(float(p.get("contracts") or 0)) <= 0:
                        continue
                    sym_p = p.get("symbol")
                    if sym_p in REGISTRY.open or sym_p in REGISTRY.pending:
                        continue
                    try:
                        await adopsi_posisi(p)
                    except Exception as exc:
                        log.error("adopsi %s gagal: %s", sym_p, exc)
            for symbol, pos in list(REGISTRY.open.items()):
                if symbol not in live:
                    try:
                        ticker = await EXCHANGE.fetch_ticker(symbol)
                        price = float(ticker.get("last") or pos["entry"])
                    except Exception:
                        price = pos["entry"]
...
# Fallback ASCII murni (terminal lama / UI_ASCII=TRUE)
EVENT_IKON_ASCII = {
    "ENTRY": "+", "WIN": "W", "LOSS": "L", "VETO": "x",
    "FAIL": "!", "BREAKEVEN": "=", "REGIME": "~", "MARKET": "#",
    "INFO": ".",
}

EVENT_STYLE = {
    "ENTRY": "bold bright_green",
...
# Warna ANSI untuk mode log (PM2) -- rich tidak dipakai di jalur ini
EVENT_ANSI = {
    "ENTRY": "\033[1;92m", "WIN": "\033[1;92m", "LOSS": "\033[1;91m",
    "VETO": "\033[1;93m", "FAIL": "\033[1;91m", "BREAKEVEN": "\033[1;96m",
    "REGIME": "\033[1;95m", "MARKET": "\033[1;94m", "INFO": "\033[0;37m",
}
ANSI_RESET = "\033[0m"
ANSI_DIM = "\033[90m"


def _ikon(kind: str) -> str:
    tabel = EVENT_IKON_ASCII if UI_ASCII else EVENT_IKON
    return tabel.get(kind, "." if UI_ASCII else "·")


def _c(kode: str) -> str:
    """Kode warna ANSI, kosong kalau warna dimatikan."""
    return "" if UI_NO_COLOR else kode
...
def push_event(kind: str, text: str):
    ...
    log.info("EVENT %s | %s", kind, text)
    if not TUI_MODE:
        garis = _c(ANSI_DIM) + ("|" if UI_ASCII else "│") + _c(ANSI_RESET)
        print(f"{_c(ANSI_DIM)}{ts}{_c(ANSI_RESET)} {garis} "
              f"{_c(EVENT_ANSI.get(kind, ''))}{_ikon(kind)} {kind:<9}{_c(ANSI_RESET)}"
              f"{garis} {text}", flush=True)
...
# --- Responsif: lebar terminal menentukan kepadatan layout --------------
UI_SPIN = "\u283b\u2839\u2838\u283c\u2834\u2826\u2827\u280f"
UI_SPIN_ASCII = "|/-\\"
SCAN_INFO = {"n": 0}
AKTIVITAS_HIST = deque(maxlen=32)

RADAR_GLYPH = {
    "posisi":   ("\u25c9", "O", "bold bright_green"),
    "sinyal":   ("\u25b2", "^", "bold green"),
    "dekat":    ("\u25cb", "o", "bold yellow"),
    "veto":     ("\u2298", "x", "yellow"),
    "cooldown": ("\u25d4", "c", "cyan"),
...
RADAR_URUT = ["posisi", "sinyal", "dekat", "gagal", "veto", "cooldown", "tutup", "pantau"]


def ui_width() -> int:
    try:
        if CONSOLE is not None:
            return max(40, int(CONSOLE.width))
    except Exception:
        pass
    return 100
...
def ui_tier() -> str:
    """wide >=140 | mid >=108 | narrow >=84 | tiny <84 kolom."""
    w = ui_width()
    if w >= 140:
        return "wide"
    if w >= 108:
...
def _kotak():
    return box.ASCII if UI_ASCII else box.ROUNDED


def _kotak_tegas():
    return box.ASCII_DOUBLE_HEAD if UI_ASCII else box.DOUBLE


def _spin() -> str:
    bingkai = UI_SPIN_ASCII if UI_ASCII else UI_SPIN
    return bingkai[SCAN_INFO["n"] % len(bingkai)]


def _sparkline(data) -> str:
    nilai = [max(0.0, float(v)) for v in data]
    if not nilai:
        return ""
    blok = ".:|#" if UI_ASCII else "\u2581\u2582\u2583\u2584\u2585\u2586\u2587\u2588"
    puncak = max(nilai) or 1.0
    return "".join(blok[min(int(v / puncak * (len(blok) - 1)), len(blok) - 1)] for v in nilai)


def _radar_kat(status: str) -> str:
    s = str(status)
    if "IN_POSITION" in s or "PENDING" in s:
        return "posisi"
    if s.startswith("\U0001f7e2") or "OPEN SIGNAL" in s:
        return "sinyal"
    if s.startswith("\U0001f6a8"):
        return "dekat"
    if s.startswith("\u274c") or "VETO" in s:
        return "veto"
    if "COOLDOWN" in s or "PAIR_CAP" in s or "DEDUP" in s or "OVERFLOW" in s:
        return "cooldown"
    if s.startswith("\U0001f319") or "CLOSED" in s:
        return "tutup"
    if s.startswith("\u26d4") or s.startswith("\u26a0") or "FAIL" in s or "ERR" in s:
        return "gagal"
    return "pantau"


def _g(kat: str):
    ikon_uni, ikon_ascii, gaya = RADAR_GLYPH.get(kat, RADAR_GLYPH["pantau"])
    return (ikon_ascii if UI_ASCII else ikon_uni), gaya
...
def _panel_header():
    tier = ui_tier()
    if DRY_RUN_MODE:
        mode = "[bold black on bright_green] DRY-RUN [/]"
    else:
        mode = "[bold white on red] LIVE [/]"
    n_tradfi = sum(1 for p in ASSET_PAIRS if is_tradfi(p))
    kiri = Text.assemble(
        ...
        (f" ({len(ASSET_PAIRS) - n_tradfi}c+{n_tradfi}m)", "dim") if tier != "tiny" else ("", ""),
        ("   ", ""),
        (f"TF {TIMEFRAME}", "cyan"),
    )
    kanan = Text.assemble(
        (datetime.now(WIB).strftime("%H:%M:%S"), "bright_white"),
        (" WIB", "dim"),
        ("   ", ""),
        (f"\u23f1 {_uptime_txt()}", "cyan"),
    )
    ...
    grid.add_column(justify="center", width=11)
    grid.add_column(justify="right", ratio=1)
    grid.add_row(kiri, mode, kanan)
    return Panel(grid, box=_kotak_tegas(), border_style="bright_cyan", padding=(0, 1))


# --- Panel: kartu ringkasan --------------------------------------------
def _kartu_ringkasan():
    s = JOURNAL.stats(24)
    pf = s["profit_factor"]
    pf_txt = "\u221e" if pf == float("inf") else f"{pf:.2f}"
    regime = MARKET_REGIME_GLOBAL["market_regime"]
    ...
    wr = s["win_rate"]
    wr_style = "bright_green" if wr >= 71 else ("yellow" if wr >= 50 else "bright_red")
    net_style = "bold bright_green" if s["pnl_usdt"] >= 0 else "bold bright_red"
    titik = {"OK": "[green]\u25cf[/]", "READY": "[dim]\u25cb[/]",
             "FALLBACK": "[yellow]\u25d0[/]", "ERROR": "[red]\u2716[/]",
             "NO KEY": "[dim]\u25cb[/]", "OFF": "[dim]\u25cb[/]"}

    kartu = []
    kartu.append(Panel(
        f"[{eq_style}]${equity_now():,.2f}[/] [dim]{EQUITY['sumber']}[/]\n"
        f"[dim]margin[/] ${margin_now():,.2f}  [dim]lev[/] {LEVERAGE}x\n"
        f"[dim]notional[/] ${notional_now():,.0f}",
        title="\U0001f4b0 EKUITAS", box=_kotak(), border_style=eq_style, padding=(0, 1)))
    kartu.append(Panel(
        f"[bold bright_white]{used}[/][dim] / {MAX_OPEN_POSITIONS} slot[/]\n"
        f"[cyan]{_bar(used, MAX_OPEN_POSITIONS)}[/]\n"
        f"[dim]cd {REENTRY_COOLDOWN_SEC // 60}m \u00b7 re-entry {REENTRY_LIMIT_TXT}[/]",
        title="\U0001f39f RISIKO", box=_kotak(), border_style="cyan", padding=(0, 1)))
    kartu.append(Panel(
        f"[{reg_style}]{reg_ikon} {regime}[/]\n"
        f"[dim]{MARKET_REGIME_GLOBAL['recommended_tactic']}[/]\n"
        f"[dim]upd {MARKET_REGIME_GLOBAL['last_updated']}[/]",
        title="\U0001f9ed REGIME", box=_kotak(), border_style=reg_style, padding=(0, 1)))
    kartu.append(Panel(
        f"[green]{s['win']}W[/] [dim]/[/] [red]{s['loss']}L[/]   "
        f"[{wr_style}]{wr:.0f}%[/]\n"
        f"[dim]PF[/] {pf_txt}   [dim]fee[/] [red]-{s.get('fee_usdt', 0.0):.2f}[/]\n"
        f"[{net_style}]{s['pnl_usdt']:+.2f} USDT[/]",
        title="\U0001f4ca 24 JAM", box=_kotak(), border_style="magenta", padding=(0, 1)))
    kartu.append(Panel(
        f"{titik.get(AI_STATUS['kimi'], '\u25cb')} Kimi   [dim]{AI_STATUS['kimi']}[/]\n"
        f"{titik.get(AI_STATUS['deepseek'], '\u25cb')} Deep   [dim]{AI_STATUS['deepseek']}[/]\n"
        f"{titik.get(AI_STATUS['llama'], '\u25cb')} Llama  [dim]{AI_STATUS['llama']}[/]",
        title="\U0001f916 ALIANSI AI", box=_kotak(), border_style="bright_magenta", padding=(0, 1)))
    kartu.append(Panel(
        f"{status_pasar_tradfi()}\n"
        f"[dim]TP {TP_PCT * 100:.2f}% \u2192 net {TP_NET_PCT * 100:.2f}%[/]\n"
        f"[dim]SL {SL_ATR_MULT}\u00d7ATR \u00b7 BE +{BREAKEVEN_OFFSET_PCT * 100:.2f}%[/]",
        title="\U0001f3db TRADFI", box=_kotak(), border_style="blue", padding=(0, 1)))
    return kartu


def _panel_stats():
    tier = ui_tier()
    kartu = _kartu_ringkasan()
    if tier == "tiny":
        s = JOURNAL.stats(24)
        ringkas = Table.grid(expand=True, padding=(0, 1))
        ringkas.add_column(justify="left")
        ringkas.add_column(justify="right")
        ringkas.add_row("[dim]ekuitas[/]", f"[bright_green]${equity_now():,.2f}[/]")
        ringkas.add_row("[dim]slot[/]",
                        f"{REGISTRY.slots_used()}/{MAX_OPEN_POSITIONS}")
        ringkas.add_row("[dim]regime[/]",
                        f"{MARKET_REGIME_GLOBAL['market_regime']}")
        ringkas.add_row("[dim]24 jam[/]",
                        f"{s['win']}W/{s['loss']}L  {s['pnl_usdt']:+.2f}")
        return Panel(ringkas, box=_kotak(), border_style="cyan", padding=(0, 1))
    per_baris = {"wide": 6, "mid": 3, "narrow": 2}[tier]
    baris = [Columns(kartu[i:i + per_baris], equal=True, expand=True)
             for i in range(0, len(kartu), per_baris)]
    return Group(*baris)


# --- Panel: posisi terbuka ---------------------------------------------
def _panel_positions():
    judul = f"\U0001f4cc POSISI TERBUKA ({len(REGISTRY.open)}/{MAX_OPEN_POSITIONS})"
    if not REGISTRY.open:
        return Panel(Align.center("[dim]\u2014 belum ada posisi terbuka \u2014[/]"),
                     title=judul, box=_kotak(), border_style="green", padding=(0, 1))
    tier = ui_tier()
    kolom = {
        "wide":   ["PAIR", "SIDE", "ENTRY", "LAST", "TP", "SL", "PnL %", "NET $", "BE", "AGE"],
        "mid":    ["PAIR", "SIDE", "ENTRY", "LAST", "PnL %", "NET $", "BE", "AGE"],
        "narrow": ["PAIR", "SIDE", "ENTRY", "PnL %", "AGE"],
        "tiny":   ["PAIR", "PnL %", "AGE"],
    }[tier]
    rata = {"PAIR": "left", "SIDE": "center", "BE": "center"}
    tabel = Table(expand=True, box=box.ASCII if UI_ASCII else box.SIMPLE_HEAD,
                  pad_edge=False, header_style="bold cyan",
                  row_styles=["", "on grey11"])
    for nama in kolom:
        tabel.add_column(nama, justify=rata.get(nama, "right"), no_wrap=True)
    for sym, p in REGISTRY.open.items():
        ...
        umur = (datetime.now(timezone.utc)
                - datetime.fromisoformat(p["opened_at"])).total_seconds() / 60.0
        panah = ("^" if UI_ASCII else "\u25b2") if p["side"] == "LONG" else ("v" if UI_ASCII else "\u25bc")
        nilai = {
            "PAIR": Text(sym.replace("/USDT:USDT", "").replace("/USDT", ""), style="bold white"),
            "SIDE": Text(f"{panah} {p['side']}",
                         style="green" if p["side"] == "LONG" else "red"),
            "ENTRY": fmt(p["entry"]), "LAST": fmt(last),
            "TP": fmt(p["tp"]), "SL": fmt(p["sl"]),
            "PnL %": _pnl_text(pnl_pct, "%"), "NET $": _pnl_text(net),
            "BE": ("[green]\u25c6[/]" if p["breakeven_done"] else "[dim]\u00b7[/]"),
            "AGE": f"{umur:.0f}m",
        }
        tabel.add_row(*[nilai[k] for k in kolom])
    return Panel(tabel, title=judul, box=_kotak(),
                 border_style="green", padding=(0, 1))


# --- Panel: radar (mode smart / full) ----------------------------------
def _panel_radar(board: dict):
    pairs = ASSET_PAIRS if DASHBOARD_ROWS <= 0 else ASSET_PAIRS[:DASHBOARD_ROWS]
    hitung = {k: 0 for k in RADAR_GLYPH}
    kats = {}
    for sym in pairs:
        kat = _radar_kat(board.get(sym, "INIT"))
        kats[sym] = kat
        hitung[kat] += 1
    menarik = [s for s in pairs if kats[s] in ("posisi", "sinyal", "dekat", "gagal", "veto")]
    tier = ui_tier()
    smart = RADAR_MODE != "full"

    ringkasan = Text()
    for kat in RADAR_URUT:
        if hitung[kat]:
            ikon, gaya = _g(kat)
            ringkasan.append(f"{ikon} {hitung[kat]:>2} {kat}   ", style=gaya)

    if smart:
        tampil = menarik[:14]
        if not tampil:
            isi = Group(
                Align.center(Text("semua pair tenang \u00b7 menunggu pemicu", style="dim italic")),
                Align.center(ringkasan),
            )
            return Panel(isi, title=f"\u26a1 RADAR \u00b7 {len(pairs)} pair \u00b7 smart",
                         box=_kotak(), border_style="bright_black", padding=(0, 1))
    else:
        tampil = pairs

    lebar_sel = 30 if tier in ("wide", "mid") else 26
    kolom = DASHBOARD_COLS if DASHBOARD_COLS > 0 else max(1, ui_width() // lebar_sel)
    baris = math.ceil(len(tampil) / kolom) or 1
    ...
    for r in range(baris):
        ...
        for c in range(kolom):
            ...
            if idx < len(tampil):
                sym = tampil[idx]
                status = str(board.get(sym, "INIT"))
                kat = kats[sym]
                ikon, gaya = _g(kat)
                nama = sym.replace("/USDT:USDT", "").replace("/USDT", "")
                sel.append(Text.assemble(
                    (f"{ikon} ", gaya),
                    (f"{nama:<5}", "bold bright_white" if is_tradfi(sym) else "white"),
                    (status[:lebar_sel - 9], gaya if kat != "pantau" else "dim"),
                ))
            else:
                sel.append("")
        grid.add_row(*sel)
    isi = Group(grid, Align.center(ringkasan))
    label = "smart" if smart else "full"
    return Panel(isi, title=f"\u26a1 RADAR \u00b7 {len(pairs)} pair \u00b7 {label}",
                 box=_kotak(), border_style="bright_black", padding=(0, 1))
...
def _panel_events():
    tier = ui_tier()
    jumlah = {"wide": 10, "mid": 8, "narrow": 6, "tiny": 4}[tier]
    potong = max(30, ui_width() - 34)
    ...
    for ts, kind, text in list(EVENT_FEED)[-jumlah:]:
        grid.add_row(f"[dim]{ts}[/]",
                     Text(_ikon(kind), style=EVENT_STYLE.get(kind, "white")),
                     Text(kind, style=EVENT_STYLE.get(kind, "white")),
                     Text(str(text)[:potong], style="grey70"))
    return Panel(grid, title="\U0001f4f0 AKTIVITAS", box=_kotak(),
                 border_style="yellow", padding=(0, 1))


# --- Panel: kaki (gerak halus) -----------------------------------------
def _panel_footer():
    spark = _sparkline(AKTIVITAS_HIST)
    bagian = Text()
    bagian.append(f"{_spin()} ", style="bright_cyan")
    bagian.append(f"scan #{SCAN_INFO['n']}", style="cyan")
    bagian.append(f" tiap {SCAN_INTERVAL:g}s", style="dim")
    if spark:
        bagian.append("   ")
        bagian.append(spark, style="bright_magenta")
        bagian.append(" aktivitas", style="dim")
    if ui_tier() != "tiny":
        bagian.append("   ")
        for kat in ("posisi", "sinyal", "dekat", "pantau"):
            ikon, gaya = _g(kat)
            bagian.append(f"{ikon} {kat}  ", style=gaya)
        bagian.append("\u00b7 Ctrl+C berhenti", style="dim italic")
    return Align.center(bagian)


def build_dashboard(board: dict):
    SCAN_INFO["n"] += 1
    aktif = sum(1 for s in board.values()
                if _radar_kat(s) in ("posisi", "sinyal", "dekat"))
    AKTIVITAS_HIST.append(aktif)
    tier = ui_tier()
    if tier == "tiny":
        return Group(_panel_header(), _panel_stats(),
                     _panel_positions(), _panel_events(), _panel_footer())
    return Group(
        _panel_header(),
        _panel_stats(),
        _panel_positions(),
...
def ui_start():
    ...
    if not TUI_MODE:
        ...
        return
    try:
        LIVE = Live(Group(), console=CONSOLE, refresh_per_second=max(1, min(UI_REFRESH, 10)), screen=True)
        LIVE.start()
    except Exception as exc:
        LIVE = None
...
def ui_render(board: dict):
    ...
    SCAN_INFO["n"] += 1
    aktif = sum(1 for v in board.values()
                if _radar_kat(v) in ("posisi", "sinyal", "dekat"))
    AKTIVITAS_HIST.append(aktif)
    dim, reset = _c(ANSI_DIM), _c(ANSI_RESET)
    tebal = _c("\033[1m")
    hijau = _c("\033[92m")
    merah = _c("\033[91m")
    used = REGISTRY.slots_used()
    warna_net = hijau if s["pnl_usdt"] >= 0 else merah
    wr = s["win_rate"]
    warna_wr = hijau if wr >= 71 else (_c("\033[93m") if wr >= 50 else merah)
    titik = {"OK": "\u25cf", "READY": "\u25cb", "FALLBACK": "\u25d0",
             "ERROR": "\u2716", "NO KEY": "\u25cb", "OFF": "\u25cb"}
    if UI_ASCII:
        titik = {k: v for k, v in
                 (("OK", "*"), ("READY", "o"), ("FALLBACK", "~"),
                  ("ERROR", "x"), ("NO KEY", "o"), ("OFF", "o"))}
    garis_h = ("-" if UI_ASCII else "\u2500") * 46
    spark = _sparkline(AKTIVITAS_HIST)
    baris = [
        f"{dim}{garis_h}{reset}",
        f"  {tebal}\U0001f493 {jam} WIB{reset}   {dim}scan #{SCAN_INFO['n']} "
        f"\u00b7 tiap {SCAN_INTERVAL:g}s \u00b7 TF {TIMEFRAME}{reset}",
        f"  {dim}slot  {reset}{_bar(used, MAX_OPEN_POSITIONS)} {used}/{MAX_OPEN_POSITIONS}"
        f"   {dim}ekuitas{reset} ${equity_now():,.2f} {dim}{EQUITY['sumber']}{reset}"
        f"   {dim}margin{reset} ${margin_now():,.2f}",
        f"  {dim}24 jam{reset} {hijau}{s['win']}W{reset}/{merah}{s['loss']}L{reset}"
        f"  {warna_wr}WR {wr:.0f}%{reset}"
        f"   {dim}net{reset} {warna_net}{s['pnl_usdt']:+.2f}{reset}"
        f"   {dim}fee{reset} {merah}-{s.get('fee_usdt', 0.0):.2f}{reset}",
        f"  {dim}AI    {reset}{titik.get(AI_STATUS['kimi'], '?')} Kimi "
        f"{titik.get(AI_STATUS['deepseek'], '?')} Deep "
        f"{titik.get(AI_STATUS['llama'], '?')} Llama"
        f"   {dim}regime{reset} {MARKET_REGIME_GLOBAL['market_regime']}"
        f"   {dim}tradfi{reset} {status_pasar_tradfi()}",
    ]
    if spark:
        baris.append(f"  {dim}radar {reset}{spark} {dim}aktivitas \u00b7 {aktif} pair menarik{reset}")
    baris.append(f"{dim}{garis_h}{reset}")
